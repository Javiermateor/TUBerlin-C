...
Canvas recursive_rectangle(Canvas c, int x, int y, int width, int height) {
    // Abbruchbedingung: Wenn die Höhe null oder negativ ist, beenden wir die Rekursion
    if (height <= 0) {
        return c;
    }
...
Canvas bucket_fill(Canvas c, int x, int y) {
    // Bestimme die Breite und Höhe der Canvas
    int canvas_w = canvas_width(c);
    int canvas_h = canvas_height(c);

    // Überprüfen, ob die Koordinaten außerhalb der Canvas liegen
